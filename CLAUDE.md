...
|-------|---------|-------------|
| `url-to-site` | `/url-to-site <url>` | **Main skill.** URL → scrape → brand → archetype → 5 variants → preview → pick → deploy |
| `cloudflare-deploy` | `/cloudflare-deploy` | Deploys a built `sites/{slug}/variants/{name}/out/` to Cloudflare Pages |
...
| `site-redesign` | `/site-redesign` | Generates single-file HTML site (original kit) |
| `vercel-deploy` | `/vercel-deploy` | Deploys to Vercel (original kit) |
